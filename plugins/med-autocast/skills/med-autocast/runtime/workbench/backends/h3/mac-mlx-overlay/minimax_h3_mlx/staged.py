...
class StrictStagedTextToVideo:
    ...
    def __init__(
        ...
        self.keep_models_loaded = bool(keep_models_loaded)
        self.verbose = bool(verbose)
        self.config = PipelineConfig.from_model_index(self.checkpoint_dir / "model_index.json")
        if self.keep_models_loaded and self.qwen_stages != 1:
            raise ValueError("Resident model caching requires one-stage Qwen loading.")
        if self.keep_models_loaded and is_streaming_checkpoint(self.transformer_dir):
            raise ValueError("Resident model caching requires a resident transformer checkpoint.")
    ...
    def encode_text(self, prompt: str) -> tuple[TextArtifact, dict[str, float]]:
        ...
        cache_hit = self._text_encoder is not None
        ...
        metrics["model_cache_hit"] = cache_hit
    ...
    def denoise(
        ...
        *,
        ...
        if self.keep_models_loaded and drop_adaln:
            raise ValueError("Cached resident DiT must retain AdaLN weights across requests.")
        model_cache_hit = self._dit is not None
        dit = self._dit
        if dit is None:
            if streaming:
        ...
        metrics = _stage_metrics(started, active)
        metrics["model_cache_hit"] = model_cache_hit
    ...
    def decode_video(
        ...
        mx.reset_peak_memory()
        started = time.perf_counter()
        model_cache_hit = self._video_vae is not None
        vae = self._video_vae
        if vae is None:
    ...
    def decode_audio(
        ...
        mx.reset_peak_memory()
        started = time.perf_counter()
        model_cache_hit = self._audio_vae is not None
        vae = self._audio_vae
        if vae is None:
