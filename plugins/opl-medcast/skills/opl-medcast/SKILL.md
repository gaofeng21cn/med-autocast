...
六阶段与原十阶段职责映射见 `docs/production-sop.md`。完整专业方法、模板和通用工具随包存于 `runtime/workbench/`；已有工作区优先使用原入口。默认交付预检使用 `medcast.py preflight-workbench`，直接读取原制作单和交付清单，不另外维护 OMC 制作状态。
