...
OPL Med Cast 0.1.0 是首个公开版本，交付源码与 Codex 插件。包含三个动作、六个工作阶段、六个专业技能，以及工作区检查、资产检索和交付预检辅助程序。

本地验证通过 14 个行为测试、OPL 标准结构检查、接口生成、源码目录检查和辅助程序定位。只读接入现有制作工作区，识别五个系列、72 张关键帧和两个关联视频拒用条目；拒用视频的残留可用区间不会进入检索结果。未重新制作媒体。

该版本尚未完成 OPL Meta Agent 独立资格验证，也未进入 OPL 的 GHCR 稳定软件包渠道。Framework 的正式软件包发布清单尚未登记 opl-medcast。GitHub Release与 Codex 插件的发布、安装，不代表 OPL 托管运行激活或视频生产验收。
