#!/usr/bin/env python3
"""Read-only Med Cast workspace inspection and deterministic preflight.
...
def inspect_workspace(root: Path) -> dict:
    ...
    return {'schema': 'opl_medcast_workspace_inspection/v1', 'read_only': True,
...
def query_assets(root: Path, category: str | None = None, series: str | None = None) -> dict:
    ...
    return {'schema': 'opl_medcast_asset_query/v1', 'read_only': True, 'count': len(assets), 'assets': assets,
...
def preflight(root: Path, delivery_ref: str, current_ref: str, review_ref: str) -> dict:
    ...
    if delivery.get('schema')!='opl_medcast_delivery/v1' or current.get('schema')!='opl_medcast_current_selection/v1':
    ...
    return {'schema':'opl_medcast_preflight/v1','read_only':True,'status':'passed',
